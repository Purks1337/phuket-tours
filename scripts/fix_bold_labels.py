...
LABELS = ("Тур включает:", "Что взять с собой:", "Примечание:")

# Normalize beginning-of-line labels like:
#   Тур включает: ...
#   **Тур включает:** ...
# into:
#   **Тур включает:** ...
LABEL_RE = re.compile(
    r"(?m)^(?P<indent>[ \t]*)(?:\*\*)?(?P<label>Тур включает:|Что взять с собой:|Примечание:)(?:\*\*)?(?P<rest>[ \t].*)?$"
)
...
def fix_text(text: str) -> tuple[str, int]:
    ...
    return LABEL_RE.subn(repl, text)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Bold labels "Тур включает:", "Что взять с собой:", "Примечание:" in markdown files.',
    )
    parser.add_argument(
        "--root",
